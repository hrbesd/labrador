//
//  SDWebImageStorageManagerStorage.m
//  Labrador
//
...
static SDWebImageStorageManager *instance;

@implementation SDWebImageStorageManager
...
+ (id)sharedManager
{
    if (instance == nil)
    {
        instance = [[SDWebImageStorageManager alloc] init];
    }
...
{
    return [[SDImageCache sharedImageStorage] imageFromKey:[self cacheKeyForURL:url]];
}

/*
 * @deprecated
 */
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageStorageManagerDelegate>)delegate retryFailed:(BOOL)retryFailed
{
 ...
 * @deprecated
 */
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageStorageManagerDelegate>)delegate retryFailed:(BOOL)retryFailed lowPriority:(BOOL)lowPriority
{
...
- (void)downloadWithURL:(NSURL *)url delegate:(id<SDWebImageStorageManagerDelegate>)delegate options:(SDWebImageOptions)options userInfo:(NSDictionary *)userInfo
{
    ...
    NSDictionary *info = [NSDictionary dictionaryWithObjectsAndKeys:
                          ...
                          nil];
    [[SDImageCache sharedImageStorage] queryDiskCacheForKey:[self cacheKeyForURL:url] delegate:self userInfo:info];
}
...
- (NSUInteger)indexOfDelegate:(id<SDWebImageStorageManagerDelegate>)delegate waitingForURL:(NSURL *)url
{
...
- (void)imageCache:(SDImageCache *)imageCache didFindImage:(UIImage *)image forKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    id<SDWebImageStorageManagerDelegate> delegate = [info objectForKey:@"delegate"];
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didUpdatePartialImage:(UIImage *)image
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        {
            id<SDWebImageStorageManagerDelegate> delegate = [downloadDelegates objectAtIndex:uidx];
            SDWIRetain(delegate);
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image
{
    ...
    if (image)
    {
        ...
        [[SDImageCache sharedImageStorage] storeImage:image
